...
particular to it. Turning on `gfm-mode` requires a some extra configuration
to set up.[^2] To do so, add the following to your Emacs configuration:

```{lisp, eval = FALSE}
(require 'poly-R) ;; make sure poly-R is loaded

;; define the gfm polymode:
...
;; register the exporters for our new polymode (same as for poly-r-markdown)
(polymode-register-exporter poly-r-markdown-ess-exporter nil
                            poly-gfm+r-polymode)
(polymode-registerj-exporter poly-r-markdown-exporter nil
                            poly-gfm+r-polymode)
...
`poly-gfm+r-mode`, which shows up in your mode bar as "PGFM-Rmd". It's
nearly identical, the main differences being the support for fenced code
blocks. 
...
[^2]: I am working on getting these changes incorporated into
[poly-R](https://github.com/polymode/poly-R/issues/21). I'll update this
tutorial when that is settled. 
